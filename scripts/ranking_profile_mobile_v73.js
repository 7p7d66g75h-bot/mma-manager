...
(function(){
  ...
  function style(){
    ...
    s.textContent=`
      /* Official mobile rankings — isolated under .mma-ranking-official */
      .ranking-profile-btn,.ranking .profile-btn,[data-action="profile"],button[onclick*="profile"]{
        min-width:110px!important;width:auto!important;min-height:44px!important;padding:10px 18px!important;
        white-space:nowrap!important;display:inline-flex!important;align-items:center!important;justify-content:center!important;box-sizing:border-box!important
      }
      ...
      .mro-record{text-align:right;border:1px solid #d8ad3d;border-radius:8px;padding:8px 10px;background:#080b0f;min-width:84px}
      .mro-record small{display:block;color:#b8a36a;font-size:9px;font-weight:900;text-transform:uppercase;letter-spacing:1px}.mro-record b{display:block;color:#f1f3f5;font-size:18px;margin-top:3px}
      ...
      .mma-ranking-official .rankline:first-of-type{border-top:1px solid #3b424b}
      ...
      .mma-ranking-official .rankline .country,.mma-ranking-official .rankline [class*=country]{color:#c7cdd4}
      .mma-ranking-official .mro-legend{display:grid;grid-template-columns:repeat(3,1fr);gap:6px;padding:10px 8px;background:#0b0f14;border-top:1px solid #2a3038}
      .mro-legend-item{background:#11161c;border:1px solid #292f37;border-radius:7px;padding:7px;text-align:center;color:#8f98a3;font-size:9px;font-weight:800}.mro-legend-item b{display:block;color:#d5ae42;font-size:13px;margin-bottom:2px}
      @media(max-width:520px){
        .mma-ranking-official .mro-banner{grid-template-columns:58px minmax(0,1fr) 76px;gap:8px;padding:11px;min-height:96px}
        .mro-belt{width:54px;height:54px;font-size:28px}.mro-name{font-size:15px}.mro-record{min-width:70px;padding:6px}.mro-record b{font-size:15px}
        .mma-ranking-official .rankline{grid-template-columns:31px minmax(0,1fr) auto!important;gap:6px!important;padding:8px 5px!important;min-height:58px}
        .mma-ranking-official .ranknum{font-size:18px!important}.mma-ranking-official .record-badge{min-width:67px!important;font-size:11px!important;padding:5px 5px!important}
        .mma-ranking-official .rankline button,.mma-ranking-official .rankline .profile-btn{min-width:70px!important;font-size:10px!important;padding:6px 7px!important}
        .mro-legend{grid-template-columns:repeat(3,1fr)!important}.mro-legend-item{font-size:8px}
      }
    `;
  ...
  function decorate(){
    ...
    const host=rows[0].closest('.panel,.card,section,div')||rows[0].parentElement;
    if(!host||host.dataset.mmaRankingOfficial==='1')return;
    host.dataset.mmaRankingOfficial='1';
    host.classList.add('mma-ranking-official');

    // Turn the first ranked fighter into a compact official champion banner.
    const first=rows[0];
    const clone=first.cloneNode(true);
    const all=clone.querySelectorAll('button');all.forEach(b=>b.remove());
    ...
    const legend=document.createElement('div');legend.className='mro-legend';
    legend.innerHTML='<div class="mro-legend-item"><b>↑</b>Поднялся</div><div class="mro-legend-item"><b>—</b>Без изменений</div><div class="mro-legend-item"><b>★</b>Претендент</div>';
  ...
  const mo=new MutationObserver(function(){style();decorate()});
