...
# Remove the unwanted V53 runtime layer. Core game code stays untouched.
...
# Keep contract history only on fighter profiles, and make the existing history panel clickable.
...
# Add one small compatibility layer. It does not change salary calculations.
patch = r"""
...
(function(){'use strict';
  ...
  function terminate59(f){
    ...
    ov.innerHTML=`<div class="v52-contract-card"><div style="font-size:46px;text-align:center">⚠️</div><div class="eyebrow" style="text-align:center">РАСТОРЖЕНИЕ КОНТРАКТА</div><div class="v51-warning-title">Расторгнуть контракт?</div><div class="v51-warning-sub">${f.name} • ${org} • осталось ${left} боёв.</div><div class="sheet-actions" style="position:static;margin-top:18px"><button id="v59TermBack">Отмена</button><button id="v59TermYes" class="red">Да, расторгнуть</button></div></div>`;
    document.body.appendChild(ov);
