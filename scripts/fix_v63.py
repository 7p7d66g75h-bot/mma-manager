...
# Remove that override so the original matchmake()/_matchmakeHeavy() pair is used again.
...
(function(){'use strict';
  // Restore the original matchmaking entry point. Do not invent a new matchmaking system here.
  // The original function is declared in the main game script and remains untouched.
  ...
  // Camp: use the existing buttons, but make their action direct and independent of
  // the older delegated handlers. No new button is created.
  document.addEventListener('click',function(e){
    const change=e.target.closest?.('[data-change-camp]');
    if(change){
      e.preventDefault();e.stopImmediatePropagation();
      const f=fighter(); if(!f)return;
      f.campChanging=true; f.campStyle=null; save(); camp();
      return;
    }
    ...
    if(style){
      e.preventDefault();e.stopImmediatePropagation();
      const f=fighter(); if(!f)return;
      f.campStyle=style.dataset.campstyle; f.preferredCampStyle=style.dataset.campstyle; f.campChanging=false; save(); camp();
      return;
    }
    const profileCamp=e.target.closest?.('[data-profile-camp]');
    if(profileCamp){
      e.preventDefault();e.stopImmediatePropagation();
      const f=fighter(); openCampFor(f); return;
    }
  },true);

  // Contract/organization history: the existing History button opens one screen.
  // Inside it, show each promotion's record calculated from completed fights.
  function ensureHistory(f){f.contractHistory=Array.isArray(f.contractHistory)?f.contractHistory:[];}
  function orgRecords(f){
    const map={};
    const hs=Array.isArray(f.history)?f.history:[];
    for(const h of hs){
      const org=h.org||h.organization||f.formerContractOrg||f.contract?.org;
      if(!org)continue;
      const x=map[org]||(map[org]={w:0,l:0,d:0,ko:0,sub:0,fights:0});
      x.fights++;
      if(h.result==='W')x.w++; else if(h.result==='L')x.l++; else x.d++;
  ...
  function renderOrgHistory(f){
    ensureHistory(f);
    const map=orgRecords(f);
    const entries=f.contractHistory||[];
    const rows=Object.entries(map).sort((a,b)=>b[1].fights-a[1].fights).map(([org,x])=>`<div class="v63-org-row"><div><b>${org}</b><small>${x.fights} боёв • ${x.ko} KO/TKO • ${x.sub} SUB</small></div><strong>${x.w}-${x.l}-${x.d}</strong></div>`).join('');
    ...
    const ov=document.createElement('div');ov.id='v63HistoryOverlay';ov.className='v52-contract-overlay';
    ov.innerHTML=`<div class="v52-contract-card"><div class="section-head"><h3>История организаций</h3><button data-v63-close>×</button></div><div class="muted" style="margin-bottom:12px">${f.name} • рекорд отдельно в каждой организации</div><section class="panel"><div class="section-head"><h3>Рекорд по организациям</h3><span>${Object.keys(map).length}</span></div>${rows||'<div class="muted">После первого боя здесь появится рекорд организации.</div>'}</section><section class="panel"><div class="section-head"><h3>История контрактов</h3><span>${entries.length}</span></div>${events||'<div class="muted">Контрактов пока нет.</div>'}</section><div class="sheet-actions" style="position:static;margin-top:14px"><button class="primary" data-v63-close2>Закрыть</button></div></div>`;
    document.body.appendChild(ov);
    const close=()=>ov.remove();ov.querySelector('[data-v63-close]').onclick=close;ov.querySelector('[data-v63-close2]').onclick=close;ov.addEventListener('click',e=>{if(e.target===ov)close()});
  }
  function historyButtonHandler(e){
    const b=e.target.closest?.('#v52HistoryPanel,.v52-history,.v51-contract-history');
    if(!b)return;
    const f=fighter(); if(!f)return;
    e.preventDefault();e.stopImmediatePropagation();
    document.getElementById('v52HistoryOverlay')?.remove();document.getElementById('v63HistoryOverlay')?.remove();
    renderOrgHistory(f);
  }
  ...
  const st=document.createElement('style');st.textContent=`
    ...
    #v52HistoryPanel,.v52-history{cursor:pointer;user-select:none;-webkit-user-select:none}
  `;document.head.appendChild(st);
  window.MMA_MANAGER_V63_FIX=true;
})();
</script>
'''
